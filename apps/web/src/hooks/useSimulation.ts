...
export function useSimulation(options?: UseSimulationOptions) {
  ...
  const runQuickEstimate = useCallback(async (
    ...
    try {
      const estimate = await simulationService.runQuickEstimate(
        decision,
        option,
        userProfile
      );
  ...
  const runSensitivityAnalysis = useCallback(async (
    ...
    try {
      ...
      const result = await simulationService.analyzeSensitivity(
        decision,
        option,
        userProfile
      );
...
export function useSimulationComparison() {
  ...
  const getComparison = useCallback((optionId: string) => {
