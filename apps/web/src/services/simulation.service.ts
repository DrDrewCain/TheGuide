...
export class SimulationService {
  ...
  async runSimulation(
    ...
    try {
      ...
      throw new Error(
        `Failed to run simulation: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    } finally {
  ...
  async runQuickEstimate(
    ...
    const result = await this.engine.runAdvancedSimulation(decision, option, userProfile, config)
  ...
  private getDataRecommendations(missingFields: string[], decisionType: DecisionType): string[] {
    ...
    if (
      (decisionType === 'career_change' || decisionType === 'job_offer') &&
      missingFields.includes('career.industry')
    ) {
