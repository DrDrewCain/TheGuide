...
import type { Scenario } from '@theguide/models'
...
export class ScenarioReducer {
  ...
  reduceScenarios(
    ...
    const { selectedIndices, newWeights } = this.fastReduction(distances, weights, targetCount)
    ...
    return {
      ...
      maxTransportCost: this.computeMaxTransportCost(distances, selectedIndices, weights),
    }
  }
  ...
  private fastReduction(
    ...
    this.redistributeWeights(distances, weights, selectedArray, newWeights)
  ...
  private wassersteinDistance(s1: Scenario, s2: Scenario): number {
    ...
    // Financial outcomes
    const horizons: ('year1' | 'year3' | 'year5' | 'year10')[] = [
      'year1',
      'year3',
      'year5',
      'year10',
    ...
    for (const horizon of horizons) {
      ...
      // Financial metrics
      distance +=
        (outcome1.financialPosition.netWorth - outcome2.financialPosition.netWorth) ** 2 / 1e10 // Normalize by typical scale

      distance += (outcome1.financialPosition.income - outcome2.financialPosition.income) ** 2 / 1e8

      // Career metrics
      distance +=
        (outcome1.careerProgress.jobSatisfaction - outcome2.careerProgress.jobSatisfaction) ** 2

      // Life metrics
      distance +=
        (outcome1.lifeMetrics.overallHappiness - outcome2.lifeMetrics.overallHappiness) ** 2
    }

    // Economic conditions
    distance += (s1.economicConditions.gdpGrowth - s2.economicConditions.gdpGrowth) ** 2 * 0.1

    distance +=
      (s1.economicConditions.inflationRate - s2.economicConditions.inflationRate) ** 2 * 0.1
   ...
   */
  private buildScenarioTree(scenarios: MultistageScenario[], stages: number): ScenarioTree {
    const root: TreeNode = {
      stage: 0,
      value: null,
      probability: 1,
    ...
    for (const scenario of scenarios) {
      ...
      for (let stage = 1; stage <= stages; stage++) {
        ...
        let childNode = currentNode.children.find(child =>
          this.valuesEqual(child.value, stageValue)
        )
   ...
   */
  private nestedDistanceReduction(tree: ScenarioTree, targetCount: number): ScenarioTree {
    ...
    for (let stage = tree.stages; stage >= 1; stage--) {
      ...
      if (nodes.length > targetCount / 2 ** (tree.stages - stage)) {
        this.reduceStageNodes(nodes, targetCount / 2 ** (tree.stages - stage))
      }
    }
  ...
  private nodeDistance(node1: TreeNode, node2: TreeNode): number {
    ...
    for (let i = 0; i < node1.value.length; i++) {
      dist += (node1.value[i] - node2.value[i]) ** 2
    }
  ...
  private mergeNodes(node1: TreeNode, node2: TreeNode): void {
    ...
    if (node1.value && node2.value) {
      for (let i = 0; i < node1.value.length; i++) {
        node1.value[i] =
          (node1.value[i] * node1.probability + node2.value[i] * node2.probability) / totalProb
      }
    }
   ...
   */
  private createDefaultReconstructor(
    template: MultistageScenario
  ): (path: number[][], probability: number) => Scenario {
    ...
    const traverse = (node: TreeNode, path: number[][], probability: number) => {
      if (node.children.length === 0) {
        ...
        for (const child of node.children) {
          traverse(child, [...path, child.value || []], probability * child.probability)
        }
      }
    }
