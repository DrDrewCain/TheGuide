...
import { SobolSequence, ScrambledSobol } from './sobol';
...
export class QMCEngine {
  ...
  generateQMCSamples(
    ...
    const n = Math.pow(2, Math.ceil(Math.log2(numSamples)));
  ...
  applyControlVariates(
    ...
    return primaryValues.map((val, i) =>
      val + beta * (controlValues[i] - controlMean)
    );
  }
  ...
  private inverseCDF(
    ...
    switch (dist.type) {
      ...
      case 'beta':
        return this.inverseBetaCDF(u, dist.alpha || 2, dist.beta || 2) * ((dist.max || 1) - (dist.min || 0)) + (dist.min || 0);
      case 'empirical':
  ...
  private betaCDF(x: number, alpha: number, beta: number): number {
    ...
    const bt = Math.exp(
      this.logGamma(alpha + beta) - this.logGamma(alpha) - this.logGamma(beta) +
