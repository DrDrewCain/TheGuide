...
export class SensitivityAnalyzer {
   ...
   */
  private transformSamples(uniformSamples: number[][], ranges: ParameterRange[]): number[][] {
  ...
  private async evaluateModel(
    ...
    for (let i = 0; i < samples.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(batch.map(sample => model(sample)))
   ...
   */
  private computeFirstOrderIndices(fA: number[], fB: number[], fC: number[][]): number[] {
    const d = fC.length
    ...
    for (let i = 0; i < d; i++) {
      ...
      const numerator = this.mean(fB.map((fb, j) => fb * (fC[i][j] - fA[j])))
   ...
   */
  private computeTotalOrderIndices(fA: number[], fB: number[], fC: number[][]): number[] {
    const d = fC.length
    const indices: number[] = []
    ...
    for (let i = 0; i < d; i++) {
      ...
      const E_i = this.mean(fA.map((fa, j) => fa * fC[i][j]))
  ...
  private computeSecondOrderIndices(
    ...
    for (let i = 0; i < d - 1; i++) {
      for (let j = i + 1; j < d; j++) {
        ...
        const fCij = fA.map((_fa, k) => {
          // This is simplified - proper implementation needs additional samples
          return fC[i][k]
        })

        const Vij = this.mean(fB.map((fb, k) => fb * (fCij[k] - fA[k]))) / varY
   ...
   */
  private assessConvergence(firstOrder: number[], totalOrder: number[]): ConvergenceMetrics {
  ...
  async morrisScreening(
    ...
    for (let t = 0; t < numTrajectories; t++) {
      ...
      const samples = this.transformMorrisTrajectory(trajectory, parameterRanges, delta)
  ...
  private generateMorrisTrajectory(d: number, levels: number): number[][] {
    ...
    const start = Array(d)
      .fill(0)
      .map(() => Math.floor(this.rng.next() * levels) / (levels - 1))
    ...
    for (const dim of order) {
      ...
      current[dim] =
        current[dim] < 0.5 ? current[dim] + 1 / (levels - 1) : current[dim] - 1 / (levels - 1)
