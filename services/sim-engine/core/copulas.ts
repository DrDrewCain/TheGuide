...
export class VineCopula {
   ...
   */
  private buildVineStructure(data: UniformData, variables: string[]): VineTree {
    ...
    for (let i = 0; i < n - 1; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        const tau = this.kendallsTau(data[var1], data[var2])
        edges.push({
          node1: var1,
          node2: var2,
          weight: Math.abs(tau),
   ...
   */
  private buildNextTreeLevel(tree: VineTree, prevLevel: number, data: UniformData): Edge[] {
   ...
   */
  private computeConditional(edge: Edge, data: UniformData, _tree: VineTree): number[] {
   ...
   */
  private fitBivariateCopula(family: CopulaFamily, u: number[], v: number[]): BivariateCopula {
  ...
  private estimateTDegrees(_u: number[], _v: number[], _rho: number): number {
...
class GaussianCopula extends BivariateCopula {
  ...
  pdf(u: number, v: number): number {
    ...
    return (
      Math.exp(-(x * x - 2 * this.rho * x * y + y * y) / (2 * factor * factor)) /
      (2 * Math.PI * factor)
    )
  }
  ...
  private bivariateNormalCDF(x: number, y: number, rho: number): number {
    ...
    return (
      this.cdfNormal(h) * this.cdfNormal(k) +
      (1 / (2 * Math.PI)) * sqrr * Math.exp(-(h * h - 2 * r * h * k + k * k) / (2 * rr))
    )
  }
...
class ClaytonCopula extends BivariateCopula {
  ...
  cdf(u: number, v: number): number {
    return (u ** -this.theta + v ** -this.theta - 1) ** (-1 / this.theta)
  }
  ...
  pdf(u: number, v: number): number {
    const factor =
      (1 + this.theta) *
      (u * v) ** (-this.theta - 1) *
      (u ** -this.theta + v ** -this.theta - 1) ** (-2 - 1 / this.theta)
    return factor
  }

  conditional(u: number, v: number): number {
    return (1 + u ** -this.theta * (v ** -this.theta - 1)) ** (-1 - 1 / this.theta)
  }
  ...
  simulate(n: number, rng: PhiloxRNG): [number[], number[]] {
    ...
    for (let i = 0; i < n; i++) {
      ...
      u.push(u1)
      v.push(
        (u2 ** (-this.theta / (1 + this.theta)) * (u1 ** this.theta - 1) + 1) ** (-1 / this.theta)
      )
    }
...
class GumbelCopula extends BivariateCopula {
  ...
  cdf(u: number, v: number): number {
    return Math.exp(
      -(((-Math.log(u)) ** this.theta + (-Math.log(v)) ** this.theta) ** (1 / this.theta))
    )
  }
  ...
  pdf(u: number, v: number): number {
    ...
    const sum = logu ** this.theta + logv ** this.theta
    const A = sum ** (1 / this.theta)

    const factor1 = (this.cdf(u, v) * (logu * logv) ** (this.theta - 1)) / (u * v)
    const factor2 = A ** -this.theta / sum
  ...
  conditional(u: number, v: number): number {
    ...
    return this.cdf(u, v) * (logv / v) ** (this.theta - 1) * sum ** (1 / this.theta - 1)
  }
  ...
  private generator(t: number): number {
    return Math.exp(-(t ** (1 / this.theta)))
  }

  private inverseGenerator(u: number): number {
    return (-Math.log(u)) ** this.theta
  }
}
...
class TCopula extends BivariateCopula {
  ...
  pdf(u: number, v: number): number {
    ...
    const factor3 = (this.nu * Math.PI) ** -1

    const Q = (x * x - 2 * this.rho * x * y + y * y) / (1 - this.rho * this.rho)
    const factor4 = (1 + Q / this.nu) ** (-(this.nu + 2) / 2)

    return (factor1 * factor2 * factor3 * factor4) / (this.pdfT(x, this.nu) * this.pdfT(y, this.nu))
  }
  ...
  conditional(u: number, v: number): number {
    ...
    const z =
      (Math.sqrt((this.nu + y * y) / (this.nu + 1)) * (x - this.rho * y)) /
      Math.sqrt(1 - this.rho * this.rho)
  ...
  private cdfT(x: number, df: number): number {
    ...
    const p = 0.5 + 0.5 * this.sign(x) * this.incompleteBeta((z * z) / (z * z + 1), 0.5, df / 2)
    return p
  }

  private pdfT(x: number, df: number): number {
    const factor1 = this.gamma((df + 1) / 2) / (Math.sqrt(df * Math.PI) * this.gamma(df / 2))
    const factor2 = (1 + (x * x) / df) ** (-(df + 1) / 2)
    return factor1 * factor2
  }

  private bivariateT(_x: number, _y: number, _rho: number, _nu: number): number {
